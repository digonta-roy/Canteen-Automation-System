...
<?php
		...
		$district=mysqli_real_escape_string($con, $_POST['idd']);
		$address=mysqli_real_escape_string( $con,$_POST['gender']);

         $emailquery="UPDATE loginstu SET name='$username',phn='$number',id='$district',gender='$address' WHERE email='$email'";


         $query = mysqli_query($con,$emailquery) or die("query faild");
         if($query)
		$qury =" select * from loginstu where email='$email' && status='active'";
		$qry = mysqli_query ($con,$qury) or die("query faild");
         { $email_pas=mysqli_fetch_assoc($qry);
            $_SESSION['username'] = $email_pas['name'];
			$_SESSION['email'] = $email_pas['email'];
			$_SESSION['nmbr'] = $email_pas['phn'];
			$_SESSION['id'] = $email_pas['id'];
			$_SESSION['gender'] = $email_pas['gender'];
         ?>
						...
						<div class="card-body ">
							...
							<form action="#" method="post">
								...
								<div class="form-group">
									...
									<input type="text" name="username" id="username" class="form-control" value="<?php echo $nm;?>" >
								...
								<div class="form-group">
									...
									<input type="text" name="idd" id="idd" class="form-control "  value="<?php echo $div;?>" >
								</div>
								<div class="form-group">
									<label for="address">Gender</label>
									<input type="text" name="gender" id="gender" class="form-control"  value="<?php echo $add;?>" >
